...
## Iterations 2-5: fixing the ULCC transfer failure

**Iteration 2 (failed): row-count minimum support.** Dropped carriers with fewer than 200 training rows. Removed 16 legitimate regional/leisure carriers (Endeavor, Alaska, JetBlue, Allegiant, etc.) but did not touch F9 (~800 rows) or NK (~700 rows), the actual culprits. Backtest MAE regressed from 13.34pp to 16.94pp. Diagnosis: F9/NK's problem is not row-count thinness but within-market share concentration.

**Iteration 3 (failed): market-diversity minimum support.** Dropped carriers appearing in fewer than 15 distinct training markets. F9 appears in 139 markets, NK in 94 — both comparable to major incumbents (AA=142, UA=144). Neither was dropped. MAE stayed flat at 13.44pp. Diagnosis: two structurally different threshold filters have now failed, confirming that the failure mode is neither absence nor thinness by any support-count measure. What's needed is shrinkage sensitive to variance in observed share across a carrier's markets.

**Iteration 4 (partial): post-fit intercept shrinkage at lambda=10.** Applied empirical-Bayes-style shrinkage: `intercept_new = intercept_old * 1 / (1 + λ * var_residual)`. F9 intercept shrunk by 51.1% (passes 40% threshold), NK by 39.1% (missed by 0.9pp). MAE improved to 10.28pp — the best of any iteration to date, clearing not just the 15pp danger line but the 10pp publishable-quality target. Stopped per pre-registered stop condition B rather than tuning λ upward without a plan.

**Iteration 5 (success): post-fit shrinkage at lambda=15.** Same shrinkage mechanism, higher strength. F9 shrunk by 61.0%, NK by 49.0% — both clear the 40% threshold. MAE 9.94pp / RMSE 17.52pp — new project best. λ=20 also passes with slightly better MAE (9.87pp) but the lower λ was selected per pre-registered tie-break rule (less aggressive shrinkage that still meets the threshold, less risk of over-shrinking correctly-fit carriers).

**Applied to ATL-SAT under the v5 model:** Delta predicted share jumps from 0.9% (v1, broken) to 23.8% (v5, working) — a 26x improvement. Still short of the observed 58.5%, but the model is now producing structurally reasonable predictions. Attribution leverage on ATL-SAT rises to 33.26% (vs. AUS-SLC's 5.74% under v1). Verdict 3 does not flip on either market — mileage and Shapley both stay GO — but the leverage numbers now provide the empirical bracket DESTER was designed to characterize.

**Central research finding:** The mileage-vs-Shapley attribution mechanism produces materially different contribution estimates on markets with substantial feed share (33% leverage on ATL-SAT at ~54% feed share), and near-negligible differences on markets where feed is small (6% leverage on AUS-SLC at 13% feed share). Same instrument, two markets, empirically bracketed. In real airline planning contexts, a 5–6x variation in attribution leverage across otherwise-comparable markets is not a rounding error — it is the size of a JV renegotiation.

**Verdict 1 rule inconsistency (documented limitation, not fixed):** ATL-SAT's Verdict 1 under v5 comes back NO_GO under the load-factor rule (expected LF 20.1% < breakeven 34.4%) while Verdicts 2 and 3 come back GO under the contribution rule (contribution positive under both attribution regimes). Root cause: Verdict 1 uses an LF-based rule inherited from AUS-SLC (small 76-seat E175, where LF was a meaningful proxy for profitability). ATL-SAT uses a 178-seat aircraft where the same rule mis-fires — the plane is structurally underloaded at 23.8% predicted share but the absolute revenue is still enough to turn contribution positive. Verdict 3 was already redefined during Layer 3 to be contribution-based; Verdict 1 was not retroactively updated. Real inconsistency, worth naming.

**Persistent limitation (not resolved by shrinkage):** 47.8% of ATL-SAT's feed itineraries still have no standalone fare data (San Antonio lacks nonstop service to most connecting endpoints), so Shapley collapses to mileage-equivalent for roughly half the feed. Independent of the logit fix, carries over unchanged. Full Shapley leverage on thin-spoke markets requires a v(B) estimation model or an alternate data source.
