"""Layer 4 Wave 2, Module 4: Verdict 2 on ATL-SAT.

Imports layer2.feed_economics.allocate_fares/feed_economics and
...
def _filter_feed_rows(chunk):
    ...
    which would also catch unrelated markets that connect through the hub or
    spoke as a waypoint (Layer 2's SLC-PIT-via-AUS class of bug)."""
...
def run_verdict2_atl_sat(db1b_csv_path, local_pax_annual, local_revenue_annual, seats_per_departure, casm_cents, distance_miles):
    print("Extracting feed itineraries through ATL...")
    ...
    feed_econ_df.to_parquet(ATL_SAT_FEED_PATH, engine="pyarrow", index=False)
    print(f"Wrote {ATL_SAT_FEED_PATH} ({len(feed_econ_df)} feed itineraries)")
